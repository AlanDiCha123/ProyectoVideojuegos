...
public class Proyectil : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            float damage = PersonajeAtaque.ObtenerDamage();
            EnemigoVida enemigoVida = enemigoObjetivo.GetComponent<EnemigoVida>();
            enemigoVida.RecibirDamage(damage);
            PersonajeAtaque.EventoEnemigoDamage?.Invoke(damage, enemigoVida);
    ...
    private void MoverProyectil()
    {
        ...
        _rigidbody2D.MovePosition(_rigidbody2D.position + direccion.normalized 
